...
library(quanteda)
# Load your data
csv_path <- 'data/boardex_director_employment_data_comp_net_2011_2023_include_everything.csv'
...
# annotations <- udpipe_annotate(model, x = corp$text)
# annotations_df <- as.data.frame(annotations)
# 
# # Filter out verbs
# non_verbs <- annotations_df[annotations_df$upos != "VERB", ]
...
```{r}
help(textmodel_seededlda) # batch size here for distributed computing
```
...
```{r}
board_employment <- read.csv("data/boardex_director_employment_data_comp_net_2011_2023_include_everything.csv")
...
```{r}
write.csv(board_employment, file = "data/boardex_director_employment_data_comp_net_2011_2023_results_with_everything.csv", row.names = FALSE)
```
